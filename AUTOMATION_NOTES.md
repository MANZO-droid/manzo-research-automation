...
- **검증한 것**: 교체 후 8/3~8/7 전 날짜 1위가 SK하이닉스(6.6~8.6조원)·
  ...
  **검증 못한 것 / 후속 조치 필요**:
  ...
  2. **`2026-07-13` 데이터가 가짜/플레이스홀더로 의심됨(미해결, 회장님
     확인 필요)** - "마켓카리에이션", "루닛타리움", "보호타이어" 등 실제
     KRX 상장사명과 다른 이름이 섞여 있고 거래대금·순매수 금액이 부자연
     스럽게 딱 떨어지는 값(예: 776억원)이다. 7/14~7/16, 7/20~7/31 구간도
     처음엔 기관/외국인 순매수가 -8440억처럼 딱 떨어지는 억단위 값이었다가
     이번 순매수 재계산(§ 위)으로 실제 값으로 교체됐는데, 7/13은 거래대금
     필드 자체가 아직도 의심스러운 값이라 순매수만 고쳐서는 안 되고 날짜
     전체를 KRX로 다시 채우거나(다른 백필 날짜처럼), 7/12(일요일)처럼
     통으로 삭제해야 할 수 있음 - 삭제는 되돌리기 어려운 작업이라 이
     대화에서는 실행하지 않음.
