...
## 2026-08-08: 순매수 금액 버그 수정 + 전체 재계산

회장님 지적: "거래대금 상위에서 개인, 기관, 외인 순매수 금액이 좀 이상해
보여." → 실제로 KODEX 200선물인버스2X(252670) 등 ETF에서 기관 순매수가
582원으로 나오는 등 명백히 잘못된 값을 확인.

- **근본 원인 1 (표 선택 인덱스)**: `fetch_investor_netbuy()`가
  `soup.select("table")[3]` 고정 인덱스로 순매수 표를 찾고 있었는데,
  ETF/ETN은 앞쪽 "주요 시세" 표가 1개뿐이라 인덱스가 밀려서 페이지네비게이션
  표(`class=Nnavi`)를 순매수 표로 잘못 읽고 있었다. `soup.find("table",
  summary=...)`로 summary 텍스트 기반 검색으로 교체(정상 종목 삼성전자로
  재검증 완료 - 총거래량 대비 기관/외국인 순매매량이 합리적인 비율로 나옴).
- **근본 원인 2 (ETF 데이터 자체 이상치)**: 표 선택을 고쳐도 일부
  레버리지/인버스 ETF·ETN(252670, 114800, 252710, 233740, 462330, 251340,
  530107, 229200, 360750, 379800 등)은 네이버 표의 거래량 컬럼 자체가
  실제 KRX 거래대금과 정확히 일치하는 값을 보여주는 이상 사례가 있어
  (원인 미상 - 네이버 페이지 자체 문제로 추정), 기관 순매매량이 총거래량의
  2배로 계산되는 등 물리적으로 불가능한 값이 나왔다. KRX 원본
  trade_amount(원)를 종가로 나눈 추정 총거래량과 비교해 순매매량이 이를
  1.5배 초과하면 저장하지 않고 0으로 처리하도록 정합성 검사 추가(가짜
  숫자를 만들어내지 않기 위함 - 0은 "계산 못 함"을 뜻하지 "순매수가
  없다"는 뜻이 아님, 다만 사이트 UI에 이 구분이 별도 표시되지는 않음).
- 커밋: `scripts/collect_gainers.py`, `scripts/backfill_krx_historical.py`
  (4b3feca).
- **전체 재계산 실행**: `scripts/fix_investor_netbuy.py` (신규, 1회성)로
  기존 `volume_stocks` 280행 전체의 `investors`를 재계산해 PATCH.
  결과 - 변경 138건(그 중 이상치→0 처리 68건, 나머지 70건은 실제 값
  보정), 동일 142건. 참고: 처음엔 `supabase_upsert()`(POST +
  `on_conflict=trade_date,rank`)로 시도했으나 이 프로젝트에서 원인 불명의
  이유로 기존 행을 못 찾고 새 행을 만들려다 `ticker` NOT NULL 위반으로
  138건 전원 실패했다(unique(trade_date, rank) 제약은 DB에 분명히 존재하는데도
  재현됨 - `db/003_volume_stocks.sql`대로면 정상 작동해야 함, 원인 미조사).
  필터 기반 PATCH(`?trade_date=eq...&rank=eq...`)로 우회해 138건 전부
  성공시켰다.
- **검증한 것**: PATCH 후 DB 재조회로 8/6·8/7 값이 실제로 바뀐 것 확인.
  삼성전자(005930)처럼 정상 표가 있는 종목은 총거래량 대비 합리적인
  순매매량으로 계산됨. 레버리지/인버스 ETF·ETN은 대부분 0/0/0으로 (전체
  280행 중 70행, 25%) 처리됨 - 잘못된 숫자보다는 안전하지만, 사이트에는
  "계산 실패"가 아니라 "0"으로만 보인다는 한계가 남음.
  **검증 못한 것 / 후속 조치 필요**:
  1. `on_conflict` upsert가 이 프로젝트에서 왜 안 통하는지 원인 미조사 -
     `scripts/collect_gainers.py`/`backfill_krx_historical.py`의 정규
     자동화 upsert(`supabase_upsert(..., "trade_date,rank")`)는 매번 전체
     컬럼을 보내서 우연히 문제가 안 드러났을 뿐일 수 있음. 재현 확인 필요.
  2. 별도로 발견한 문제(이번 수정과 무관, 미해결): `volume_stocks`에서
     삼성전자 등 대형주가 특정 날짜(8/3 #8, 8/4 #8, 8/6 #9)에 거래대금이
     2천만~3천만원대(실제로는 조 단위여야 함)로 저장된 행이 존재 -
     `fetch_volume_stocks()`가 네이버 `sise_quant.naver` 페이지를 스크래핑할
     때 특정 상황에서 잘못된 셀을 읽은 것으로 추정되나 원인 미확인. 이
     대화에서는 다루지 않음.

## 2026-08-08 (이어서): 위 "삼성전자 거래대금 오류"의 진짜 정체 - 근본 버그 발견

회장님이 사이트 화면에서 "개인/기관/외국인 순매수 칸이 전부 비어 있다"는
스크린샷을 보여주며 지적 → 8/7 거래대금 상위 10위가 전부 저가 레버리지·
인버스 ETF/ETN(KODEX 200선물인버스2X 1위 94억원 등)이었던 게 이상해서
재조사. 위 §"삼성전자 거래대금 오류"로 별건 취급했던 문제가 사실은 이
전체 현상과 같은 뿌리였다.

- **근본 원인**: `fetch_volume_stocks()`가 네이버 `sise_quant.naver` 표에서
  `tds[5]`(거래량, 주식수)를 `tradeAmount`로 읽고 있었다. 실제 컬럼 순서는
  순위,종목명,현재가,전일비,등락률,**거래량**,**거래대금**(백만원 단위),
  매수호가,... - 즉 한 칸 밀려서 읽고 있었던 것. 게다가 이 페이지 자체가
  기본적으로 **거래량순 정렬**이라(제목 자체도 "거래상위 종목"이지 "거래대금
  상위"가 아니었음 - URL이 `sise_quant`라 착각하기 쉬움), 저가 레버리지·
  인버스 ETF가 항상 1~10위를 독점했다. 삼성전자가 어쩌다 8~9위에 낀 날은
  삼성전자의 **거래량(주식수, 2천만~3천만주)**이 거래대금으로 잘못 저장돼
  위 §의 "삼성전자 거래대금 오류"처럼 보였던 것 - 두 문제가 하나였다.
  - 실증: 라이브 페이지에서 KODEX 200선물인버스2X는 거래량순 1위(9,434,794,071)
    지만 실제 거래대금(6번째 컬럼×백만)은 9,094억원인 반면, SK하이닉스는
    거래량순 38위에 그쳤지만 실제 거래대금은 6.94조원으로 전체 1위였다.
- **수정**: `tds[6]`(거래대금, 백만원 단위)을 읽어 ×1,000,000 환산하도록
  교체. 이 페이지가 거래량순이라 한 페이지(최대 100행) 전체를 모은 뒤
  거래대금 기준으로 다시 정렬해 진짜 top10을 뽑도록 함(기존 20개 컷 제거).
  커밋 `a3039e9`.
- **영향 범위 확인 및 재백필**: `volume_stocks` 전체 날짜 중 1위 거래대금이
  1000억원 미만인 날짜만 감사 → 2026-08-03~08-07(5일, 실제 자동화가
  이 버그를 갖고 돌아간 기간으로 추정)이 해당. `scripts/fix_volume_ranking.py`
  (신규, 1회성)로 이 5일을 KRX Open API(진짜 ACC_TRDVAL)로 다시 계산해
  10행씩 통째로 교체(단순 필드 patch가 아니라 top10 구성 종목 자체가
  바뀌므로 DELETE 후 INSERT). 이어서 prevRank/prevTradeAmount도 교체된
  데이터 기준으로 재계산해 채움.
- **검증한 것**: 교체 후 8/3~8/7 전 날짜 1위가 SK하이닉스(6.6~8.6조원)·
  삼성전자(4.7~6.9조원) 등 실제로 합리적인 대형주로 나옴. 라이브
  `/api/top-gainers` 응답(2026-08-08 실제 호출)으로 8/7 1위=SK하이닉스
  7.25조원, investors={foreign:-801억,individual:333억,institution:468억}
  실제 채워져 나오는 것 확인.
  **검증 못한 것 / 후속 조치 필요**:
  1. 이 버그가 정확히 언제부터 daily_automation에 실렸는지(즉 8/3 이전
     날짜들은 왜 영향이 없는지) 미조사 - 아마 그 이전 날짜들은 전부
     `backfill_krx_historical.py`(KRX 원본, 이 버그와 무관)로 채워졌고
     8/3부터 정규 일일 자동화(`fetch_volume_stocks`)가 실제로 돌기
     시작했을 것으로 추정되나 커밋/실행 로그로 확정하지 않음.
  2. **`2026-07-13` 가짜/플레이스홀더 데이터 - 해결됨(2026-08-08)**:
     "마켓카리에이션", "루닛타리움" 등 실제 KRX 상장사명과 다른 이름이
     섞여 있고 거래대금·순매수 금액이 부자연스럽게 딱 떨어지는 값이던
     문제. 회장님 확인 후 `python scripts/backfill_krx_historical.py
     --dates 2026-07-13`로 KRX 원본(월요일, 정상 거래일)으로 daily_gainers·
     volume_stocks 전체 10행씩 다시 채움 - 콘텐트리중앙·STX그린로지스·
     기가레인 등 실제 KRX 상장 종목명, SK하이닉스 15.7조원 등 합리적인
     거래대금으로 교체됨. 뉴스가 실제로 없는 6종목은 정직하게 "뉴스 기사를
     수집하지 못했습니다"로 남기고(가짜 뉴스 생성 안 함) `patch_gainer_fields.py
...
## 2026-08-08 (이어서): 차트에 이동평균선(5/20/60/120일) 추가 + 전체 소급 반영

회장님 요청 "차트 그림에 5일선, 20일선, 60일선, 120일선을 같이 넣어줄
수 있어?" → 이어서 "나머지 과거 리포트도 전부 이동평균선 반영해줘".

- **백엔드(이 저장소)**: `calc_ma_lines()`(신규, `scripts/collect_gainers.py`) -
  화면에 보여줄 최근 60일 각각에 대해 그 시점 기준 이동평균(5/20/60/120일)을
  계산해 `technicals.maLines`로 저장. 기존 `calc_technicals`의 ma5~ma120은
  최신 시점 값 1개뿐이라 선을 그릴 수 없었음. ma120선이 60일 구간 앞부분까지
  안 끊기려면 그보다 120일 더 이전 데이터가 필요해 OHLCV 수집 깊이를
  120일→200일로 늘림(`run_daily`/`run_weekly`/`backfill_date`/
  `recompute_weekly`/`fix_excluded_gainers.py` 전부 반영). 커밋 `9497c47`.
- **프론트엔드(사이트 저장소, `만조인베스트 웹페이지 관리`)**: `makeRealCandleSVG()`에
  `maLines` 인자 추가 - 5일(주황 `#E67E22`)/20일(보라 `#8E44AD`)/60일(초록
  `#27AE60`)/120일(회색 `#555566`) 4색 선 + 우상단 범례. 값이 없는 구간
  (신규상장 등 데이터 부족)은 억지로 잇지 않고 끊기게 둠(지어내지 않음).
  카드뷰·모달뷰 양쪽 적용. 부수 수정: 모달 라벨이 "250일 일봉"이었지만
  실제로는 60일치만 저장돼 표시와 실제가 어긋났던 기존 불일치를 "일봉"으로
  정정. 커밋 `dc3351d`, **회장님 승인 받고 푸시 → 라이브 배포 완료**(회장님이
  직접 라이브 사이트 확인).
- **신규**: `scripts/patch_ma_lines.py` - 지정 날짜의 daily_gainers를
  200일 OHLCV로 다시 계산해 technicals(추세·이격도·ADX·maLines 포함)만
  PATCH(ohlcv/chart_analysis/rise_reason은 안 건드림 - 차트 선 추가가
  목적이라 문구 재생성은 범위 밖).
- **소급 반영 실행**: 1차 2026-08-03~08-08 60행(시범, 60/60 성공) →
  2차 나머지 23개 날짜(07-02~08-01) 230행(226/230 성공, 4건 스킵).
  **결과: 전체 daily_gainers 290행 중 286행(98.6%) maLines 반영 확인**
  (직접 재조회로 확인, `technicals.maLines` 존재 여부 검사).
- **스킵된 4건과 원인**: 2026-07-14 #1 신진에스엠(002860), #3 빅텐츠(323490),
  2026-07-16 #2 비엔에스웍스(436380), #6 씨에스윈드(312450) - 네이버
  fchart API가 빈 응답(`<protocol />`)을 반환해 조사해보니, KRX Open API
  전종목 목록(`stk_isu_base_info`/`ksq_isu_base_info`)에 이 4개 종목코드가
  아예 없음(상장폐지 또는 종목코드 변경으로 추정) - 재조회가 불가능한
  진짜 데이터 부재이지 코드 버그가 아님. 값을 지어내지 않고 그대로 둠
  (해당 4건은 기존 candlestick 차트만 표시되고 이동평균선은 안 보임).

## 2026-08-09: 새 추세 기준(정배열/역배열+이격도+ADX)을 과거 chartAnalysis 문구 전체에 소급 반영 - 진행 중(중단)

회장님 요청: "이 내용[정배열/역배열+이격도+ADX]을 과거 데이터 전체에
반영해줘." `patch_trend_technicals.py`(7/23·7/24에 이미 썼던 것)를
나머지 27개 날짜(290행 - 20행)에 돌리는 작업. **주의**: 이 스크립트가
maLines 백필(위 섹션) 이전에 만들어져 OHLCV를 120일만 가져오고
maLines를 안 채웠던 걸 발견 - 그대로 돌리면 방금 채운 maLines가
사라질 뻔해서, 먼저 200일 수집 + `calc_ma_lines()` 호출을 추가해
maLines를 유지하도록 고쳤다(커밋 `e9c6473`).

**여러 차례 시도, 반복적으로 할당량 문제에 부딪힘**:
1. 로컬 Gemini로 27개 날짜(270행) 시도 → 4건만 성공, 나머지 429 폭탄 →
   중단(3.5분씩 재시도만 반복하며 사실상 멈춰 있었음).
2. GH Actions Groq로 같은 270행 시도 → **60분 타임아웃으로 강제
   취소**(`gh run view --json status` 폴링으로 확인, `--log`로 재확인) -
   89건 성공/7건 실패/나머지 미처리. GH Actions의 `timeout-minutes: 60`이
   이렇게 큰 배치엔 부족함을 처음 확인.
3. 이어서 GH Actions Groq로 9개 날짜(90행) 재시도 → 이번에도 60분
   타임아웃, 이번엔 처리된 9건 중 8건이 Groq 429 - Groq도 오늘 누적
   사용으로 할당량이 빠듯해진 것으로 추정.
4. Gemini가 오후 4시(태평양 자정) 리셋 시각을 지났길래 단발 테스트
   호출로 정상 응답 확인 → 로컬에서 18개 날짜(180행) 재개했지만 이번에도
   금방 429 폭탄(2건 성공/22건 실패) → 중단.
5. 로그 4개(local 2회 + GH Actions 2회)를 모아 `[OK]` 라인을 파싱해
   실제로 어떤 (날짜, 순위)가 성공했는지 정확히 집계 → **290행 중 111행
   완료, 179행 남음**(07-13 일부 + 07-14~08-08 전체).
6. 남은 것 중 07-13 10행만 다시 테스트 → **0/10 완전 실패** - Gemini도
   지금은 사실상 막혀 있음을 확인하고 무리한 재시도를 멈춤.

**교훈**: (1) 무료 티어 하루 총사용량이 이 대화 하루 동안 이미 상당히
소진된 상태라 두 provider 모두 신뢰할 수 없는 상태였다 - 단발 테스트
호출 성공이 배치 전체의 성공을 보장하지 않는다(RPD와 RPM을 구분해서
봐야 함). (2) `technicals.disparity`/`adx` 키 존재 여부로 "새 기준
반영됨"을 판단하면 안 된다 - `calc_technicals()`가 이제 항상 이 값들을
포함하므로, maLines 백필처럼 technicals만 갱신하고 chartAnalysis 문구는
안 건드린 행도 이 키가 있는 것처럼 보여 오판할 뻔했다(실제로는 로그
기반으로 [OK] 라인을 집계해야 정확함). (3) GH Actions
`timeout-minutes: 60`은 200행 이상 배치에는 부족 - 다음엔 처음부터
50~80행 단위로 쪼개서 돌릴 것.

**다음에 이어서 할 일 - 자동화로 해결(2026-08-09 저녁)**: 회장님이
"한국시간 내일 오후 4시 이후에 다시 시도해보자"고 하셔서, 수동으로
매번 재시도하는 대신 구조적으로 해결했다:

1. `scripts/patch_trend_technicals.py`에 `technicals.trendCriteriaVersion=2`
   완료 마커 추가 - 이미 문구가 새로 생성된 행은 `--dates`에 다시 넣어도
   자동으로 건너뛴다(멱등적 재실행 가능). 오늘 실제 성공 확인된 111행
   (로그 파싱으로 집계)에는 LLM 재호출 없이 마커만 미리 심어둠. `--minutes`
   시간 예산 옵션도 추가(예산 소진 시 스스로 멈추고 다음 실행이 이어받음).
   커밋 `80291fe`.
2. `.github/workflows/backfill-krx.yml`에 `schedule: cron: "0 7 * * *"`
   (매일 UTC 07:00 = 한국시간 16시, Gemini 할당량 리셋 직후) 추가 -
   `github.event_name == 'schedule'`이면 남은 19개 날짜를
   `patch_trend_technicals.py --provider groq --minutes 45`로 자동 실행.
   마커 덕분에 매일 돌아도 안전(다 끝나면 조회만 하고 아무 것도 안 함).
   커밋 `ede7549`.
3. 남은 19개 날짜(179행):
...
**사람이 확인할 것**: 179행이 다 끝났는지는 아래로 확인 가능 -
```
select count(*) from daily_gainers where (technicals->>'trendCriteriaVersion')::int = 2;
```
`290`이 나오면 전부 끝난 것 - 그러면 `.github/workflows/backfill-krx.yml`의
`schedule:` 블록(3줄)을 지워서 매일 도는 걸 멈추면 된다(안 지워도 무해하지만
불필요한 실행이 계속 남음). Groq도 할당량이 부족하면 이 예약 실행이 매일
조금씩만 진행될 수 있음 - 그때는 `provider`를 gemini로 바꿔서 수동
`workflow_dispatch`로 한 번 더 밀어주는 것도 방법.

## 2026-08-12: 추세 기준 소급 반영 완료 + 08-10 리포트 공백 발견·복구

**추세 기준 소급 반영 - 완료(자동 재시도로 해결됨)**: 예약 실행이
8/9~8/12 사흘간 자동으로 돌면서 처리:
- 8/9: 111행 (수동, 로그 파싱 집계)
- 8/10: +68행 (성공 68/실패 10/이월 101, 45분 예산 소진)
- 8/11: +65행 (성공 65/실패 11/이월 35)
- 8/12: +42행 (성공 42/실패 4, 시간 예산 안에서 완주 - 처음으로 이월 0건)

최종 286/290행(98.6%) 완료. **남은 4행은 진짜 불가능한 케이스로 확정**:
2026-07-14 #1 신진에스엠(002860), #3 빅텐츠(323490), 2026-07-16 #2
비엔에스웍스(436380), #6 씨에스윈드(312450) - 매일 재시도했지만 매번
"OHLCV 없음"으로 스킵됨(위 §의 maLines 백필 때 이미 KRX 전종목 목록에
없다고 확인한 것과 동일한 4종목 - 상장폐지 추정, 값을 지어낼 수 없음).
목적 달성으로 `backfill-krx.yml`의 `schedule:` 트리거 제거(patch_mode=trend는
`workflow_dispatch`로 여전히 수동 실행 가능).

**08-10(월요일, 정상 거래일) daily_gainers 리포트 공백 발견**: 회장님이
"8월 10일 리포트가 없어"라고 지적 → `상승률 상위 10위 자동 수집`
워크플로(gainers-daily.yml) 08-10 실행 기록이 `failure`였던 것 확인.
로그 확인 결과 - 상승률 9위 종목(롯데웰푸드) 분석 중 Groq 429가 4회
재시도(12→10→13→16초 대기) 후에도 안 풀려서 **스크립트가 의도적으로
전체 실행을 중단**("무료 할당량이 소진된 것으로 보여 자동 실행을
중단합니다" - `collect_gainers.py`의 기존 설계, 부분 결과를 저장하지
않고 통째로 실패 처리해 조용한 데이터 누락을 막는 방식). 즉 8/8~8/11
사이에 뜬 8/9(일요일, 원래 없음)와 별개로 8/10만 진짜 공백이었음.

**복구**: `backfill_krx_historical.py --dates 2026-08-10`으로 KRX
원본 데이터 재수집 - daily_gainers·volume_stocks 각 10행 정상 채움
(1위 윙입푸드 +29.99% 등). 검증: DB 재조회로 8/10 10행 존재 확인,
32개 날짜 연속성(8/4~8/12, 8/9 제외) 확인.
**후속 조치 필요**: gainers-daily.yml이 이런 식으로 조용히 하루를
