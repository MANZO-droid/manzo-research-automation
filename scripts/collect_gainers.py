...
# 2026-08-12 확장: 일본어만 걸러내다 보니 태국어(สำค)·베트남어(tương)·
# 중국어 한자(反应) 같은 다른 언어 혼입은 못 잡고 있었다(회장님이 옵시디언
# 볼트로 옮기다가 발견 - daily_gainers 310행 중 205행에서 발견). 한국어
# 텍스트에 나올 일이 없는 문자 범위를 최대한 넓게 잡는다. 다만 "masih",
# "hareket"처럼 영문자만으로 이뤄진 인도네시아어/터키어 단어는 문자 범위로는
# 구분이 안 돼 이 검사로 못 잡는다(알려진 한계).
_FOREIGN_SCRIPT_RE = re.compile(
    r"[぀-ヿ"          # 일본어 히라가나/가타카나
    r"฀-๿"    # 태국어
    r"一-鿿"    # 중국어/한자(한국어 실사용 텍스트엔 거의 안 나옴)
    r"ƀ-ɏ"    # 라틴 확장(베트남어 ư/ơ 등)
    r"Ḁ-ỿ"    # 라틴 확장 추가(베트남어 성조 결합문자 등)
    r"]"
)


def has_language_issue(text: str) -> bool:
    """Groq(llama-3.3-70b)/Gemini가 "반드시 순수 한국어로만 작성" 지시를
    가끔 무시하고 다른 언어로 응답하는 사례가 실제로 여러 건 확인됐다
    (2026-08-04/05 일본어, 2026-08-12 태국어/베트남어/중국어 한자 - 회장님
    지적). 한국어에 나올 일 없는 문자 범위 존재를 언어 오염의 신호로 쓴다."""
